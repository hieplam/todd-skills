...
only that campaign and no unrelated local state.

```sh
$ cp -R ~/.tribe/<e2e-repo-key> <scratch>/before-tribe-root/
...
Result: `before-status-page.png`, PNG 1600x900. One campaign row plus a
`Session tail - C1-<sessionId>.log (159295 bytes)` block: 40 lines of raw runner JSONL in a
