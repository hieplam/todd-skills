...
| **02 — this card** | — | — |
| **03 — decorrelate via INPUT asymmetry (contract-lens vs cold-lens)** | **The one card that deliberately *narrows* the allowlist, and must not be misread as widening it.** Idea 03's "cold lens" reviewer is given the diff **without** the contract. That is a *subtraction* from my allowlist (drop item 1 for reviewer B) — perfectly compatible. The rule to preserve when 03 lands: the allowlist is a **ceiling, not a floor**; a variant may show a reviewer *less*, never *more*, and never anything from the banned list. **This card should be worded so that constraint reads naturally** — hence "may contain **only**", framing it as a maximum. | **No conflict, if the ceiling framing is kept.** Flagged as a wording constraint on the plan's rule text. |
...
**Net:** zero conflicts across all nine. One **hard dependency inbound** (01 is worthless without this),
one **interlock verified against the sibling's committed spec** (05), one **wording constraint** to
carry into the plan (03: the allowlist must read as a **ceiling**, so a later card may show a reviewer
*less* but never *more*), and one **boundary question** to hand the Shaman for 06 (contract-class vs
narrative-class CODEX sections).
