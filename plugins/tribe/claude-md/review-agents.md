...
- **While developing / before every commit or PR** → `tracker` agent (rules reviewer, formerly code-reviewer). Question: "does this diff follow our written rules?" It reads every rule source fresh (global rules, CLAUDE.md, formatter/linter config, C3), checks the diff, and flags correctness bugs with a concrete fix per finding. Its verdict is advisory (BLOCK / APPROVE-WITH-COMMENTS / APPROVE) — run it often as the cheap recurring gate. It also surfaces harness gaps — a diff-anchored, risk-scoped, ≥3-file pattern no written rule covers — as a separate read-only section that is never a violation, never invented, and never judged by Tracker itself.
